...
export function analyzeResume(text, fileName = "") {
  ...
  const missingSections = RESUME_SECTIONS.filter((s) => !lower.includes(s));
  ...
  // 6. Email / phone / URL presence (contact info)
  ...
  let experience = Math.min(20, foundVerbs.length * 2 - badCount * 2);
  ...
  const total = keywords + formatting + skills + experience + readability;
  const atsScore = Math.max(25, Math.min(98, total));

  // ── Build feedback arrays ──────────────────────────────────────────────────
  const strengths = [];
  ...
  if (missingSections.includes("summary")) improvements.push("Add a professional summary at the top to quickly capture recruiter attention.");
  if (missingSections.includes("projects")) improvements.push("Include a Projects section with links to demonstrate practical skills.");
  if (missingSections.includes("certifications")) improvements.push("Consider adding certifications to strengthen your profile.");
  ...
  if (wordCount < 150) improvements.push("Resume seems too short — consider expanding your experience and projects sections.");
  if (wordCount > 1000) improvements.push("Resume may be too long for ATS systems — try to condense to 1-2 pages.");

  const summary = atsScore >= 80
    ? "Your resume is performing well for ATS systems. Focus on tailoring keywords for each specific job description."
    : atsScore >= 60
    ? "Your resume has a decent foundation. A few targeted improvements will significantly boost your ATS score."
    : "Your resume needs improvement for ATS systems. Follow the suggestions below to increase your chances of passing initial screening.";
