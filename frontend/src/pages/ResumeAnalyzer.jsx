...
export default function ResumeAnalyzer() {
  ...
  const analyzeResume = async () => {
    ...
    try {
      ...
      const mapped = {
        ...
        score_breakdown: {
          content:            data.scores?.keywords ?? 0,
          format:             data.scores?.formatting ?? 0,
          skills:             data.scores?.skills ?? 0,
          experience:         data.scores?.experience ?? 0,
          overall_impression: data.scores?.readability ?? 0,
        },
  ...
  const scoreLabels = ["Keywords", "Formatting", "Skills", "Experience", "Readability"];
